...
TEST_F(LimitOrderFillTest, PartialFillsWithOrderBook) {
    ...
    auto order_id = order_manager_->submit_limit_order("AAPL", qse::Order::Side::BUY, 150, 100.0,
    ...
    EXPECT_LT(
        order_manager_->get_cash(),
        100000.0); // Should have spent money on the order (initial cash is 100000 from config)
}

TEST_F(LimitOrderFillTest, LimitOrderDoesNotFillWhenPriceNotCrossed) {
    // Submit a limit buy order for 100 shares at price 99.0
    auto order_id = order_manager_->submit_limit_order("AAPL", qse::Order::Side::BUY, 100, 99.0,
                                                       qse::Order::TimeInForce::GTC);
...
TEST_F(LimitOrderFillTest, SellOrderPartialFills) {
    ...
    auto sell_order_id = order_manager_->submit_limit_order("AAPL", qse::Order::Side::SELL, 150,
                                                            100.2, qse::Order::TimeInForce::GTC);
...
TEST_F(LimitOrderFillTest, OrderBookLiquidityConsumption) {
    ...
    auto order_id = order_manager_->submit_limit_order("AAPL", qse::Order::Side::BUY, 200, 100.0,
                                                       qse::Order::TimeInForce::GTC);
