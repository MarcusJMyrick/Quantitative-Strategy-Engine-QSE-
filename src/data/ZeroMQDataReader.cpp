...
ZeroMQDataReader::ZeroMQDataReader(const std::string& endpoint, int timeout_ms)
    : endpoint_(endpoint), timeout_ms_(timeout_ms), data_loaded_(false),
...
void ZeroMQDataReader::start_receiving() const {
    ...
    try {
        ...
        data_loaded_ = true;
        ZMQ_LOG(std::cout << "Data reception completed. Received " << received_ticks_.size()
                          << " ticks." << std::endl;);
...
ZeroMQDataReader::convert_timestamp(int64_t timestamp_s) const {
