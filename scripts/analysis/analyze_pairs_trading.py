...
def analyze_pairs_trading_trades(trade_data):
    ...
    for symbol_pair, df in trade_data.items():
        ...
        symbol1_volume = (
            symbol1_trades["quantity"].sum() if has_quantity and not symbol1_trades.empty else 0
        )
        symbol2_volume = (
            symbol2_trades["quantity"].sum() if has_quantity and not symbol2_trades.empty else 0
        )
...
def create_simple_pairs_analysis(trade_df, symbol_pair, output_dir):
    ...
    plt.scatter(
        symbol1_buy_trades.index,
        symbol1_buy_trades["price"],
        color="green",
        marker="^",
        s=50,
        label=f"{symbol1} Buy",
    ...
    plt.scatter(
        symbol1_sell_trades.index,
        symbol1_sell_trades["price"],
        color="red",
        marker="v",
        s=50,
        label=f"{symbol1} Sell",
        alpha=0.7,
    )
    ...
    plt.tight_layout()
    plt.savefig(
        output_path / f"{symbol1}_trade_prices_{symbol_pair}.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
    ...
    plt.scatter(
        symbol2_buy_trades.index,
        symbol2_buy_trades["price"],
        color="green",
        marker="^",
        s=50,
        label=f"{symbol2} Buy",
    ...
    plt.scatter(
        symbol2_sell_trades.index,
        symbol2_sell_trades["price"],
        color="red",
        marker="v",
        s=50,
        label=f"{symbol2} Sell",
        alpha=0.7,
    )
    ...
    plt.tight_layout()
    plt.savefig(
        output_path / f"{symbol2}_trade_prices_{symbol_pair}.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(
        output_path / f"combined_trade_activity_{symbol_pair}.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
    ...
    if "quantity" in trade_df.columns:
        ...
        plt.subplot(3, 1, 1)
        plt.plot(
            symbol1_trades.index,
            symbol1_trades["quantity"],
            color="green",
            alpha=0.7,
            linewidth=1,
            label=f"{symbol1} Volume",
        )
        ...
        plt.subplot(3, 1, 2)
        plt.plot(
            symbol2_trades.index,
            symbol2_trades["quantity"],
            color="blue",
            alpha=0.7,
            linewidth=1,
            label=f"{symbol2} Volume",
        )
        ...
        if "timestamp" in trade_df.columns:
            ...
            plt.plot(
                symbol1_sorted["timestamp"],
                symbol1_cumulative,
                color="green",
                linewidth=2,
                label=f"{symbol1} Cumulative Volume",
            )
            plt.plot(
                symbol2_sorted["timestamp"],
                symbol2_cumulative,
                color="blue",
                linewidth=2,
                label=f"{symbol2} Cumulative Volume",
            )
        ...
        width = 0.35
        plt.bar(
            x - width / 2, volume_stats[symbol1], width, label=symbol1, alpha=0.7, color="green"
        )
        ...
        plt.tight_layout()
        plt.savefig(
            output_path / f"volume_statistics_{symbol_pair}.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
    ...
    plt.tight_layout()
    plt.savefig(
        output_path / f"price_distributions_{symbol_pair}.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
...
def create_pairs_trading_plots(equity_data, trade_data, analysis, output_dir):
    ...
    for symbol_pair in trade_data.keys():
        ...
        if has_side and not symbol1_trades.empty:
            ...
            print(
                f"⚠️  Skipping buy/sell markers for {symbol_pair}: missing 'side' column or no trades"
            )
        ...
        if has_quantity and has_symbol:
            symbol1_trades["position"] = (
                symbol1_trades["quantity"].cumsum() if not symbol1_trades.empty else 0
            )
            symbol2_trades["position"] = (
                symbol2_trades["quantity"].cumsum() if not symbol2_trades.empty else 0
            )
            plt.plot(
                symbol1_trades["timestamp"],
                symbol1_trades["position"],
                label=f"{symbol1} Position",
                linewidth=2,
            )
            plt.plot(
                symbol2_trades["timestamp"],
                symbol2_trades["position"],
                label=f"{symbol2} Position",
                linewidth=2,
            )
        else:
            print(
                f"⚠️  Skipping position tracking for {symbol_pair}: missing 'quantity' or 'symbol' column"
            )
        ...
        if has_quantity and has_symbol and "timestamp" in trade_df.columns:
            ...
            daily_volume = (
                trade_df.groupby(["date", "symbol"])["quantity"].sum().unstack(fill_value=0)
            )
            ...
            print(
                f"⚠️  Skipping trade volume plot for {symbol_pair}: missing 'quantity', 'symbol', or 'timestamp' column"
            )
        ...
        if has_pnl and "timestamp" in trade_df.columns:
            ...
            if has_symbol:
                ...
                plt.plot(
                    symbol1_trades["timestamp"], symbol1_pnl, label=f"{symbol1} P&L", alpha=0.7
                )
                plt.plot(
                    symbol2_trades["timestamp"], symbol2_pnl, label=f"{symbol2} P&L", alpha=0.7
                )
        ...
        if "timestamp" in trade_df.columns:
            ...
            print(
                f"⚠️  Skipping trade distribution plot for {symbol_pair}: missing 'timestamp' column"
            )
        ...
        if "spread" in trade_df.columns:
            ...
            print(
                f"⚠️  Skipping spread/trade size plot for {symbol_pair}: missing 'spread' and 'quantity' columns"
            )
        ...
        if "timestamp" in trade_df.columns:
            ...
            plt.plot(
                trade_df["timestamp"],
                trade_df["rolling_trades"],
                label="20-Trade Rolling Window",
                linewidth=2,
                color="red",
            )
        ...
        plt.tight_layout()
        plt.savefig(
            output_path / f"pairs_trading_analysis_{symbol_pair}.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
...
def main():
    ...
    parser.add_argument(
        "--organized-dir", required=True, help="Path to organized results directory"
    )
    parser.add_argument(
        "--output-dir",
