...
def process_raw_tick_data(symbol: str) -> str | None:
    ...
    try:
        ...
        if not input_file.exists():
            logger.error(
                f"Input file {input_file} not found. Please run download_data.py for this symbol first."
            )
            return None
        ...
        logger.error(
            f"An unexpected error occurred while processing tick data for {symbol}: {str(e)}"
        )
        return None
