...
public:
    ...
    MOCK_METHOD(void, execute_buy, (const std::string& symbol, int quantity, double price),
                (override));
    MOCK_METHOD(void, execute_sell, (const std::string& symbol, int quantity, double price),
                (override));
    ...
    MOCK_METHOD(void, record_equity,
                (long long timestamp, (const std::map<std::string, double>&)market_prices),
                (override));
    // --- Tick-level order management mock methods ---
    MOCK_METHOD(qse::OrderId, submit_market_order,
                (const std::string& symbol, qse::Order::Side side, qse::Volume quantity),
                (override));
    MOCK_METHOD(qse::OrderId, submit_limit_order,
                (const std::string& symbol, qse::Order::Side side, qse::Volume quantity,
                 qse::Price limit_price, qse::Order::TimeInForce tif),
                (override));
    ...
    MOCK_METHOD(std::optional<qse::Order>, get_order, (const qse::OrderId& order_id),
                (const, override));
    MOCK_METHOD(std::vector<qse::Order>, get_active_orders, (const std::string& symbol),
                (const, override));
};
...
TEST_F(SMACrossoverStrategyTest, GeneratesBuySignalOnGoldenCross) {
    ...
    std::vector<double> prices = {100, 99, 98, 97, 96, 95, 94, 93, 92,  91,  90, 89, 88, 87,
                                  86,  85, 84, 83, 82, 81, 80, 79, 78,  77,  76, 75, 74, 73,
                                  72,  71, 70, 69, 68, 67, 66, 65, 64,  63,  62, 61, 60, 59,
                                  58,  57, 56, 55, 54, 53, 52, 51, 105, 106, 107};
    ...
    EXPECT_CALL(mock_order_manager, execute_sell("SPY", 1, ::testing::DoubleEq(expected_close)))
        .Times(1);
...
TEST_F(SMACrossoverStrategyTest, NoSignalOnFlatPrices) {
    ...
    EXPECT_CALL(mock_order_manager, execute_sell(::testing::_, ::testing::_, ::testing::_))
        .Times(0);
...
TEST_F(SMACrossoverStrategyTest, IgnoresBarsForWrongSymbol) {
    ...
    std::vector<double> prices = {100, 99, 98, 97, 96, 95, 94, 93, 92,  91,  90, 89, 88, 87,
                                  86,  85, 84, 83, 82, 81, 80, 79, 78,  77,  76, 75, 74, 73,
                                  72,  71, 70, 69, 68, 67, 66, 65, 64,  63,  62, 61, 60, 59,
                                  58,  57, 56, 55, 54, 53, 52, 51, 105, 106, 107}; // Same sequence
                                                                                   // as golden
                                                                                   // cross test
    for (double price : prices) {
        qse::Bar bar;
...
TEST_F(SMACrossoverStrategyTest, GeneratesSellSignalOnDeathCross) {
    ...
    std::vector<double> prices = {110, 109, 108, 107, 106, 105, 104, 103, 102, 101, 100, 99, 98, 97,
                                  96,  95,  94,  93,  92,  91,  90,  89,  88,  87,  86,  85, 84, 83,
                                  82,  81,  80,  79,  78,  77,  76,  75,  74,  73,  72,  71, 70, 69,
                                  68,  67,  66,  65,  64,  63,  62,  61,  95,  94,  93};
    ...
    // 2. Expectation: We expect the execute_sell method to be called exactly once at the expected
    // price.
    EXPECT_CALL(mock_order_manager, execute_sell("SPY", 1, ::testing::DoubleEq(expected_close)))
        .Times(1);
