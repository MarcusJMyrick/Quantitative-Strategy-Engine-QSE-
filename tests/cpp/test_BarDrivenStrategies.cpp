...
protected:
    void SetUp() override { mock_order_manager = std::make_shared<MockOrderManager>(); }
...
TEST_F(PairsTradingStrategyBarTest, EntersShortPositionOnHighZScore) {
    ...
    for (auto& [bar_a, bar_b] : bars) {
        std::cout << "DEBUG: Feeding bar_a: " << bar_a.symbol
                  << " ts=" << qse::to_unix_ms(bar_a.timestamp) << " close=" << bar_a.close
                  << std::endl;
        strategy->on_bar(bar_a);
        std::cout << "DEBUG: Feeding bar_b: " << bar_b.symbol
                  << " ts=" << qse::to_unix_ms(bar_b.timestamp) << " close=" << bar_b.close
                  << std::endl;
        strategy->on_bar(bar_b);
    }
...
protected:
    void SetUp() override {
        ...
        strategy = std::make_unique<SMACrossoverStrategy>(mock_order_manager.get(),
                                                          20,   // Short window
                                                          50,   // Long window
                                                          "SPY" // Symbol
        );
...
TEST_F(SMACrossoverStrategyBarTest, DoesNothingBeforeWarmup) {
    ...
    for (int i = 0; i < 4; ++i) {
        Bar bar{"SPY", qse::from_unix_ms(1000 + i), 100.0 + i, 100.0 + i, 100.0 + i, 100.0 + i,
                1000};
        strategy->on_bar(bar);
    }
}
...
TEST_F(SMACrossoverStrategyBarTest, GeneratesBuySignalOnGoldenCross) {
    ...
    for (int i = 0; i < 50; ++i) { // Updated from 5 to 50
        Bar bar{"SPY", qse::from_unix_ms(1000 + i), 100.0 + i, 100.0 + i, 100.0 + i, 100.0 + i,
                1000};
        dbg << "BAR ts=" << to_unix_ms(bar.timestamp) << " open=" << bar.open
    ...
    Bar bar51{"SPY", qse::from_unix_ms(1050), 105.0, 105.0, 105.0, 105.0,
              1000}; // Updated timestamp
...
TEST_F(SMACrossoverStrategyBarTest, GeneratesSellSignalOnDeathCross) {
    ...
    for (size_t i = 0; i < prices.size(); ++i) {
        Bar bar{"SPY", qse::from_unix_ms(1000 + i), prices[i], prices[i], prices[i], prices[i],
                1000};
        strategy->on_bar(bar);
    }
}
