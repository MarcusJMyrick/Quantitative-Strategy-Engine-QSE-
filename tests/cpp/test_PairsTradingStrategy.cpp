...
protected:
    void SetUp() override {
        ...
        strategy =
            std::make_unique<PairsTradingStrategy>("ASSET_A", "ASSET_B",
                                                   2.0, // Hedge ratio
...
TEST_F(PairsTradingStrategyTest, EntersShortPositionOnHighZScore) {
    strategy = std::make_unique<PairsTradingStrategy>("ASSET_A", "ASSET_B",
                                                      2.0, // hedge ratio
    ...
    //    μ ≈ 0, σ small ⇒ z ≫ 2
    EXPECT_CALL(*mock_order_manager, get_position("ASSET_A")).WillOnce(Return(0));
...
TEST_F(PairsTradingStrategyTest, ExitsPositionOnMeanReversion) {
    ...
    EXPECT_CALL(*mock_order_manager, get_position("ASSET_A")).WillOnce(Return(100));
    EXPECT_CALL(*mock_order_manager, get_position("ASSET_B")).WillOnce(Return(-200));
    ...
    // Revert ASSET_B to slightly higher price to trigger exit (spread = 100 - 2*50.1 = -0.2, z <
    // 0.5)
...
TEST_F(PairsTradingStrategyTest, WorksWithLowerThreshold) {
    ...
    test_strategy->on_bar(
        Bar{"ASSET_A", qse::from_unix_ms(1000), 100.0, 100.0, 100.0, 100.0, 1000});
    test_strategy->on_bar(Bar{"ASSET_B", qse::from_unix_ms(1000), 50.0, 50.0, 50.0, 50.0, 1000});

    test_strategy->on_bar(
        Bar{"ASSET_A", qse::from_unix_ms(1001), 100.0, 100.0, 100.0, 100.0, 1000});
    test_strategy->on_bar(Bar{"ASSET_B", qse::from_unix_ms(1001), 50.0, 50.0, 50.0, 50.0, 1000});

    test_strategy->on_bar(
        Bar{"ASSET_A", qse::from_unix_ms(1002), 100.0, 100.0, 100.0, 100.0, 1000});
    test_strategy->on_bar(Bar{"ASSET_B", qse::from_unix_ms(1002), 50.0, 50.0, 50.0, 50.0, 1000});

    test_strategy->on_bar(
        Bar{"ASSET_A", qse::from_unix_ms(1003), 100.0, 100.0, 100.0, 100.0, 1000});
    test_strategy->on_bar(Bar{"ASSET_B", qse::from_unix_ms(1003), 50.0, 50.0, 50.0, 50.0, 1000});

    test_strategy->on_bar(
        Bar{"ASSET_A", qse::from_unix_ms(1004), 100.0, 100.0, 100.0, 100.0, 1000});
    ...
    test_strategy->on_bar(
        Bar{"ASSET_A", qse::from_unix_ms(1005), 300.0, 300.0, 300.0, 300.0, 1000});
...
TEST_F(PairsTradingStrategyTest, BasicFunctionality) {
    ...
    // Now try a large deviation - based on debug output, this triggers at price 150 with ASSET_B at
    // 49
