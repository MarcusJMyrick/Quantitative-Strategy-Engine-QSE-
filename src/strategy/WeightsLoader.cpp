...
WeightsLoader::load_weights_from_file(const std::string& file_path) {
    ...
    try {
        ...
        if (res.ok()) {
            ...
            auto reader_res =
                arrow::csv::TableReader::Make(arrow::io::IOContext(arrow::default_memory_pool()),
                    ...
                    if (table) {
                        ...
                        if (symbol_col && weight_col) {
                            for (int chunk = 0; chunk < symbol_col->num_chunks(); ++chunk) {
                                auto sym_arr = std::static_pointer_cast<arrow::StringArray>(
                                    symbol_col->chunk(chunk));
                                auto w_arr = std::static_pointer_cast<arrow::DoubleArray>(
                                    weight_col->chunk(chunk));
        ...
        if (weights.empty()) {
            ...
            while (std::getline(ifs, line)) {
                ...
                if (std::getline(ss, symbol, ',') && std::getline(ss, weight_str, ',')) {
                    try {
                        ...
                        if (std::isfinite(weight) && std::abs(weight) <= 10.0) {
                            weights[symbol] = weight;
                            std::cerr << "[DEBUG] Loaded weight: " << symbol << " = " << weight
                                      << std::endl;
                        } else {
                            std::cerr << "[DEBUG] Skipped invalid weight: " << symbol << " = "
                                      << weight_str << std::endl;
                        }
            ...
            for (const auto& pair : weights) {
                std::cerr << "[DEBUG] Final weight: " << pair.first << " = " << pair.second
                          << std::endl;
            }
