...
PortfolioBuilder::optimize(const std::vector<double>& alpha_scores,
...
PortfolioBuilder::OptimizationResult
PortfolioBuilder::optimize_from_table(const std::shared_ptr<arrow::Table>& factor_table,
                                      const std::string& alpha_col, const std::string& beta_col,
    ...
    if (alpha_chunked->num_chunks() != 1 || beta_chunked->num_chunks() != 1 ||
        symbol_chunked->num_chunks() != 1) {
...
double PortfolioBuilder::compute_portfolio_beta(const Eigen::VectorXd& weights,
                                                const Eigen::VectorXd& beta) {
    return weights.dot(beta);
}

bool PortfolioBuilder::check_constraints(const Eigen::VectorXd& weights,
                                         const Eigen::VectorXd& beta) {
...
Eigen::VectorXd PortfolioBuilder::project_to_constraints(const Eigen::VectorXd& weights,
                                                         const Eigen::VectorXd& beta) {
    ...
    if (std::abs(portfolio_beta - config_.beta_target) > config_.beta_tolerance) {
        ...
        if (beta_norm_sq > 1e-9) {
            ...
            if (std::abs(beta_dot_adjusted_beta) > 1e-9) {
                double beta_adjustment =
                    (portfolio_beta - config_.beta_target) / (beta.dot(beta_adjusted));
...
void PortfolioBuilder::save_weights(const OptimizationResult& result,
    ...
    auto schema = arrow::schema(
        {arrow::field("symbol", arrow::utf8()), arrow::field("weight", arrow::float64())});
