...
def analyze_pairs_aggressive():
    ...
    print(
        f"    - Exit threshold: 0.5 → {len(valid_data[abs(valid_data['z_score']) < 0.5])} signals"
    )
    ...
    sample = merged.tail(20)[
        ["timestamp", f"{symbol1}_close", f"{symbol2}_close", "spread", "z_score"]
    ]
    ...
    for _, row in sample.iterrows():
        ...
        if abs(z_score) > entry_threshold:
            ...
            print(
                f"  {timestamp} | {row[f'{symbol1}_close']:.2f} | {row[f'{symbol2}_close']:.2f} | {row['spread']:.2f} | {z_score:.3f} {signal_type}"
            )
        elif abs(z_score) < exit_threshold:
            print(
                f"  {timestamp} | {row[f'{symbol1}_close']:.2f} | {row[f'{symbol2}_close']:.2f} | {row['spread']:.2f} | {z_score:.3f} 🟡 EXIT"
            )
        else:
            print(
                f"  {timestamp} | {row[f'{symbol1}_close']:.2f} | {row[f'{symbol2}_close']:.2f} | {row['spread']:.2f} | {z_score:.3f}"
            )
    ...
    if total_signals > 0:
        print(
            f"✅ With aggressive parameters, you should see {total_signals} PairsTrading signals!"
        )
        print(
            f"   This is {total_signals/len(valid_data)*100:.1f}% of all bars - very active trading!"
        )
