...
def print_detailed_summary_table(results, timestamp):
    ...
    print(
        f"{'Symbol':<12} {'Strategy':<15} {'Initial':<12} {'Final':<12} {'Return%':<10} {'Sharpe':<8} {'MaxDD%':<8} {'Trades':<8}"
    )
    print("-" * 100)
    ...
    for r in sorted_results:
        print(
            f"{r['symbol']:<12} {r['strategy']:<15} ${r['initial_value']:<11,.0f} ${r['final_value']:<11,.0f} "
    ...
    for symbol in symbols:
        ...
        if symbol_results:
            ...
            print(
                f"{symbol:<12} {best['strategy']:<15} {best['total_return']:>8.2f}% "
