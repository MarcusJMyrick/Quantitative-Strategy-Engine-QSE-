...
MultiFactorStrategy::MultiFactorStrategy(const std::vector<std::string>& symbols,
                                         int momentum_window, int momentum_short_window,
                                         int volatility_window, int value_window,
                                         int rebalance_frequency, int top_n, int bottom_n,
...
void MultiFactorStrategy::on_bar(const Bar& bar) {
    ...
    for (const auto& symbol : symbols_) {
        if (price_history_[symbol].size() <
            std::max({momentum_window_, volatility_window_, value_window_})) {
    ...
    for (const auto& symbol : symbols_) {
        ...
        current_factors_[symbol] =
            FactorData(symbol, bar.timestamp, momentum, volatility, value, 0.0);
    }
...
double MultiFactorStrategy::compute_momentum_factor(const std::string& symbol) {
    ...
    if (price_history_[symbol].size() >= momentum_short_window_) {
        double old_price =
            price_history_[symbol][price_history_[symbol].size() - momentum_short_window_];
        double current_price = price_history_[symbol].back();
...
double MultiFactorStrategy::compute_volatility_factor(const std::string& symbol) {
    ...
    int start_idx =
        std::max(0, static_cast<int>(return_history_[symbol].size()) - volatility_window_);
    ...
    double mean_return =
        std::accumulate(recent_returns.begin(), recent_returns.end(), 0.0) / recent_returns.size();
...
FactorReturns MultiFactorStrategy::perform_cross_sectional_regression() {
    ...
    double mean_momentum =
        std::accumulate(momentum_exposures.begin(), momentum_exposures.end(), 0.0) /
        momentum_exposures.size();
    double mean_volatility =
        std::accumulate(volatility_exposures.begin(), volatility_exposures.end(), 0.0) /
        volatility_exposures.size();
    double mean_value = std::accumulate(value_exposures.begin(), value_exposures.end(), 0.0) /
                        value_exposures.size();
    ...
    return FactorReturns(Timestamp(), momentum_return, volatility_return, value_return,
                         mean_return);
}
...
void MultiFactorStrategy::update_price_history(const std::string& symbol, double price) {
    ...
    size_t max_history =
        std::max({static_cast<size_t>(momentum_window_), static_cast<size_t>(volatility_window_),
                  static_cast<size_t>(value_window_)}) +
        10;
...
void MultiFactorStrategy::execute_portfolio_changes(
    const std::vector<PortfolioWeights>& target_weights) {
    if (!order_manager_) {
        return;
    }
