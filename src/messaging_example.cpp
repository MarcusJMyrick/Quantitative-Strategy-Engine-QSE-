...
void run_subscriber() {
    try {
        ...
        tick_subscriber.set_tick_callback([](const Tick& tick) {
            std::cout << "Received tick: price=" << tick.price << ", volume=" << tick.volume
                      << std::endl;
        });

        bar_subscriber.set_bar_callback([](const Bar& bar) {
            std::cout << "Received bar: " << bar.symbol << " O:" << bar.open << " H:" << bar.high
        ...
        std::thread tick_thread([&]() { tick_subscriber.listen(); });

        std::thread bar_thread([&]() { bar_subscriber.listen(); });
