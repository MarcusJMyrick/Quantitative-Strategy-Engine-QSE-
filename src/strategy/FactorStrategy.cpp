...
FactorStrategy::FactorStrategy(std::shared_ptr<IOrderManager> order_manager, std::string symbol,
                               const std::string& weights_dir, double min_dollar_threshold,
                               const ExecConfig& engine_config)
    : order_manager_(std::move(order_manager)), symbol_(std::move(symbol)),
      weights_dir_(weights_dir), min_dollar_threshold_(min_dollar_threshold),
...
void FactorStrategy::on_day_close(const Timestamp& timestamp) {
    ...
    if (new_weights) {
        ...
        std::cout << "[FACTOR] Loaded " << target_weights_.size()
                  << " weights for date: " << WeightsLoader::date_to_string(timestamp) << std::endl;
    } else {
...
void FactorStrategy::on_day_close_with_prices(
    const Timestamp& timestamp, const std::unordered_map<std::string, double>& close_prices) {
...
void FactorStrategy::compute_and_submit_delta_orders(
    ...
    for (const auto& [sym, w_target] : target_weights_) {
        ...
        if (std::fabs(d_w * nav) < min_dollar_threshold_)
            continue;
