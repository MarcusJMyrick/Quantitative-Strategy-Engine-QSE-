...
def find_cointegrated_pairs(data: pd.DataFrame, significance_level: float = 0.05) -> List[dict]:
    ...
    for symbol1, symbol2 in pairs:
        ...
        if (
            data[symbol1].isna().sum() > len(data) * 0.1
            or data[symbol2].isna().sum() > len(data) * 0.1
        ):
            continue
        ...
        try:
            ...
            if p_value < significance_level:
                ...
                print(
                    f"✓ Found cointegrated pair: {symbol1}-{symbol2} (p={p_value:.4f}, β={hedge_ratio:.4f})"
                )
...
def print_results(ranked_pairs: List[dict], output_file: Optional[str] = None):
    ...
    # Print header
    print(
        f"{'Rank':<4} {'Pair':<15} {'P-Value':<10} {'Hedge Ratio':<12} {'R²':<8} {'Corr':<8} {'Quality':<8}"
    )
    print("-" * 80)
    ...
    for i, pair_info in enumerate(ranked_pairs, 1):
        ...
        print(
            f"{i:<4} {pair_str:<15} {pair_info['p_value']:<10.4f} "
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Find cointegrated pairs for pairs trading strategy"
    )
    ...
    parser.add_argument(
        "--start-date", default="2020-01-01", help="Start date for data download (YYYY-MM-DD)"
    )
    parser.add_argument(
        "--end-date", default="2024-01-01", help="End date for data download (YYYY-MM-DD)"
    )
    parser.add_argument(
        "--significance",
        type=float,
