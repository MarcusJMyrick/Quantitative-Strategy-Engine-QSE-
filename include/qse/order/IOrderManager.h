...
public:
    ...
    // A mechanism for the backtester to update the equity curve, providing all current market
    // prices.
    virtual void record_equity(long long timestamp,
                               const std::map<std::string, double>& market_prices) = 0;
};
