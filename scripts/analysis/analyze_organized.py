    ...
    for file_path in equity_files:
        try:
            ...
            if not df.empty:
                ...
                if (
                    filename == "equity_AAPL_current"
                    or filename == "equity_GOOG"
                    or filename == "equity_MSFT"
                    or filename == "equity_SPY"
                ):
    ...
    for file_path in trade_files:
        try:
            ...
            if not df.empty:
                ...
                # Handle special cases
                if (
                    filename == "tradelog_AAPL"
                    or filename == "tradelog_GOOG"
                    or filename == "tradelog_MSFT"
                    or filename == "tradelog_SPY"
                ):
                    # These are simple files without strategy info
...
def create_performance_plots(equity_data, metrics, output_dir):
    ...
    if metrics:
        ...
        heatmap_data = metrics_df[
            ["total_return_pct", "volatility_pct", "sharpe_ratio", "max_drawdown_pct"]
        ]
    ...
    with open(output_path / "analysis_report.txt", "w") as f:
        ...
        if metrics:
            ...
            for key, metric in metrics.items():
                f.write(
                    f"{key:<30} {metric['total_return_pct']:<10.2f} {metric['volatility_pct']:<8.2f} "
        ...
        if trade_analysis:
            f.write(
                f"{'Strategy':<30} {'Trades':<8} {'Buy':<6} {'Sell':<6} {'Volume':<10} {'P&L':<12}\n"
            )
            f.write("-" * 70 + "\n")
            for key, analysis in trade_analysis.items():
                f.write(
                    f"{key:<30} {analysis['total_trades']:<8} {analysis['buy_trades']:<6} "
