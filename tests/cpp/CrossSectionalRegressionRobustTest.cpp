...
static std::shared_ptr<arrow::Table> build_reg_table(int n, double b1, double b2, double noise_sd) {
    ...
    std::shared_ptr<arrow::Array> a1, a2, ar;
    x1b.Finish(&a1);
    x2b.Finish(&a2);
    yb.Finish(&ar);
    auto schema =
        arrow::schema({arrow::field("f1", arrow::float64()), arrow::field("f2", arrow::float64()),
...
TEST(CrossSectionalRegressionRobustTest, HandlesOutliersWithWinsorize) {
    ...
    // Inject outlier
    arrow::DoubleBuilder rb;
    rb.Append(1000.0); // huge return
    std::shared_ptr<arrow::Array> out_arr;
    rb.Finish(&out_arr); // just placeholder
