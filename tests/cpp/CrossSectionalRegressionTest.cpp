...
TEST(CrossSectionalRegressionTest, OLSRegressionSimple) {
    ...
    for (double v : factor1)
        factor1_builder.Append(v);
    for (double v : factor2)
        factor2_builder.Append(v);
    for (double v : returns)
        return_builder.Append(v);
    ...
    std::vector<std::shared_ptr<arrow::Field>> schema_fields = {
        arrow::field("factor1", arrow::float64()), arrow::field("factor2", arrow::float64()),
        arrow::field("returns", arrow::float64())};
    auto schema = std::make_shared<arrow::Schema>(schema_fields);
    std::vector<std::shared_ptr<arrow::Array>> columns = {factor1_array, factor2_array,
                                                          return_array};
