...
TEST(DiffCalcTest, ExistingPositions) {
    ...
    std::unordered_map<std::string, double> prices = {{"AAPL", 150.0}, {"GOOG", 2500.0}};
...
TEST(DiffCalcTest, MissingPriceData) {
    ...
    std::unordered_map<std::string, double> target_weights = {
        {"AAPL", 0.1}, {"UNKNOWN", 0.1} // No price data for this symbol
    };
...
TEST(DiffCalcTest, ZeroPriceHandling) {
    ...
    std::unordered_map<std::string, double> target_weights = {
        {"AAPL", 0.1}, {"ZERO_PRICE", 0.1} // Zero price
    };
    ...
    std::unordered_map<std::string, double> prices = {
        {"AAPL", 150.0}, {"ZERO_PRICE", 0.0} // Invalid price
    };
...
TEST(DiffCalcTest, NAVCalculationEdge) {
    ...
    std::unordered_map<std::string, double> prices = {
        {"AAPL", 150.0}, {"GOOG", 2500.0}, {"TSLA", 200.0}, {"NFLX", 400.0}};
...
TEST(DiffCalcTest, BuildOrdersTargetPercentStyle) {
    ...
    cfg.min_qty = 0;
    FactorExecutionEngine engine(cfg, nullptr);
    std::unordered_map<std::string, long long> target_qty = {{"AAPL", 100}, {"GOOG", -50}};
    std::unordered_map<std::string, double> target_weights = {{"AAPL", 0.12}, {"GOOG", -0.07}};
    auto orders = engine.build_orders(target_qty, target_weights);
    ASSERT_EQ(orders.size(), 2);
    for (const auto& order : orders) {
...
TEST(DispatcherTest, CallsOM) {
    ...
    qse::Order o1;
    o1.symbol = "AAPL";
    o1.type = qse::Order::Type::MARKET;
    o1.side = qse::Order::Side::BUY;
    o1.quantity = 100;
    qse::Order o2;
    o2.symbol = "GOOG";
    o2.type = qse::Order::Type::MARKET;
    o2.side = qse::Order::Side::SELL;
    o2.quantity = 50;
    orders.push_back(o1);
    orders.push_back(o2);
...
TEST(DispatcherTest, TargetPercentLogsWarning) {
    ...
    qse::Order o1;
    o1.symbol = "AAPL";
    o1.type = qse::Order::Type::TARGET_PERCENT;
    o1.side = qse::Order::Side::BUY;
    o1.quantity = 100;
    o1.target_percent = 0.12;
    orders.push_back(o1);
...
TEST(DispatcherTest, UnsupportedTypeExplicitError) {
    ...
    qse::Order o2;
    o2.symbol = "GOOG";
    o2.type = qse::Order::Type::LIMIT;
    o2.side = qse::Order::Side::SELL;
    o2.quantity = 50;
    o2.limit_price = 150.0;
    orders.push_back(o1);
    orders.push_back(o2);
