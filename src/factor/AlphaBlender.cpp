...
AlphaBlender::BlendingResult
AlphaBlender::blend_factors(const std::shared_ptr<arrow::Table>& table,
...
double AlphaBlender::calculate_ir(const std::vector<double>& factor_values,
    ...
    double denominator = std::sqrt((sum_f2 - n * mean_f * mean_f) * (sum_r2 - n * mean_r * mean_r));
...
AlphaBlender::calculate_ir_weights(const std::shared_ptr<arrow::Table>& table,
                                   const std::vector<std::string>& factor_cols,
                                   const std::string& return_col, const std::string& date_col) {
    ...
    for (const auto& factor_ir : factor_irs) {
        ...
        double weight = std::max(config_.min_ir_weight, std::min(config_.max_ir_weight, ir_abs));
...
std::map<std::string, double>
AlphaBlender::normalize_weights(const std::map<std::string, double>& weights) {
...
AlphaBlender::apply_weights(const std::shared_ptr<arrow::Table>& table,
                            const std::vector<std::string>& factor_cols,
