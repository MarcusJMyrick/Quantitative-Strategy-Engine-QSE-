...
protected:
    void SetUp() override {
        ...
        sma_strategy = std::make_unique<SMACrossoverStrategy>(mock_order_manager.get(),
                                                              3,    // Short window
                                                              5,    // Long window
                                                              "SPY" // Symbol
        );
        ...
        ON_CALL(*mock_order_manager, execute_buy(_, _, _))
            .WillByDefault([](const std::string&, int, double) {});
        ON_CALL(*mock_order_manager, execute_sell(_, _, _))
            .WillByDefault([](const std::string&, int, double) {});
    }
...
TEST_F(BarDrivenIntegrationTest, SyntheticTickDataIntegration) {
    ...
    for (const auto& tick : ticks) {
        ...
        if (tick.symbol == "SPY") {
            if (auto bar = bar_builder_spy->add_tick(tick)) {
                dbg << "SPY BAR ts=" << to_unix_ms(bar->timestamp) << " open=" << bar->open
            ...
            if (auto bar = bar_builder_ivv->add_tick(tick)) {
                dbg << "IVV BAR ts=" << to_unix_ms(bar->timestamp) << " open=" << bar->open
                    << " high=" << bar->high << " low=" << bar->low << " close=" << bar->close
                    << " vol=" << bar->volume << "\n";
    ...
    if (auto bar = bar_builder_spy->flush()) {
        dbg << "SPY FLUSH BAR ts=" << to_unix_ms(bar->timestamp) << " open=" << bar->open
            << " high=" << bar->high << " low=" << bar->low << " close=" << bar->close
            << " vol=" << bar->volume << "\n";
        pairs_strategy->on_bar(*bar);
        sma_strategy->on_bar(*bar);
    ...
    if (auto bar = bar_builder_ivv->flush()) {
        dbg << "IVV FLUSH BAR ts=" << to_unix_ms(bar->timestamp) << " open=" << bar->open
            << " high=" << bar->high << " low=" << bar->low << " close=" << bar->close
            << " vol=" << bar->volume << "\n";
        pairs_strategy->on_bar(*bar);
    }
