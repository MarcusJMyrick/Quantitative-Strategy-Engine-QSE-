...
protected:
    void SetUp() override { order_manager = std::make_shared<StrictMock<MockOrderManager>>(); }
...
TEST_F(FactorStrategyRebalanceGuardTest, NoDuplicateOrders) {
    ...
    auto strategy = std::make_unique<FactorStrategy>(order_manager, "TEST", test_dir, 1.0);
    ...
    std::unordered_map<std::string, double> close_prices = {{"AAPL", 100.0}, {"MSFT", 100.0}};
...
TEST_F(FactorStrategyRebalanceGuardTest, MinDollarThreshold) {
    ...
    auto strategy = std::make_unique<FactorStrategy>(order_manager, "TEST", test_dir,
                                                     50.0); // $50 minimum threshold
    ...
    // Create close prices map
    std::unordered_map<std::string, double> close_prices = {{"AAPL", 100.0}};
