...
void run_backtest(const BacktestConfig& config) {
    try {
        ...
        qse::Backtester backtester(config.symbol, // <-- The missing argument
                                   std::move(data_reader), std::move(strategy),
...
int main() {
    try {
        ...
        std::cout << "----------------------------------------" << std::endl;
        std::cout << "Total execution time for all backtests: " << elapsed_time.count() << " ms"
                  << std::endl;
        std::cout << "----------------------------------------" << std::endl;
