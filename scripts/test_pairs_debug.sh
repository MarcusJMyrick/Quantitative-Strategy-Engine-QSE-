...
echo "🔍 PairsTrading Debug Test"
echo "=========================="

# Create a simple test to verify PairsTrading strategy
cat > test_pairs_debug.cpp << 'EOF'
#include <iostream>
#include <memory>
...
# Build the test
echo "Building PairsTrading debug test..."
cd build
cmake ..
make -j$(nproc)
cd ..

if [ -f "build/test_pairs_debug" ]; then
    echo "✅ PairsTrading debug test built successfully"
    
    echo "Running PairsTrading debug test..."
    ./build/test_pairs_debug
    
    # Clean up
    rm -f test_pairs_equity.csv test_pairs_trades.csv
    rm -f test_pairs_debug.cpp
else
    echo "❌ Failed to build PairsTrading debug test"
fi 