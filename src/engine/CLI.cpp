...
void print_usage(const std::string& program_name) {
    std::cout
        << "Usage: " << program_name << " run --strategy factor [OPTIONS]\n"
        << "\n"
        << "Options:\n"
...
bool parse_command_line(int argc, char* argv[], std::string& config_file, std::string& data_file,
                        std::string& symbol, std::string& weights_dir, double& min_threshold,
...
int run_factor_strategy(const std::string& config_file, const std::string& data_file,
                        const std::string& symbol, const std::string& weights_dir,
                        double min_threshold, const std::string& rebalance_time) {
    ...
    try {
        ...
        std::cout << "  Weights directory: "
                  << (weights_dir.empty() ? config.get_weights_directory() : weights_dir)
                  << std::endl;
        ...
        auto order_manager = std::make_shared<qse::OrderManager>(
            config.get_initial_cash(), "equity_curve.csv", "tradelog.csv");

        // Create factor strategy
        auto strategy = std::make_unique<qse::FactorStrategy>(
            order_manager, symbol, config.get_weights_directory(),
            config.get_min_dollar_threshold(), config.to_exec_config());

        // Create backtester
        qse::Backtester backtester(symbol, std::move(data_reader), std::move(strategy),
                                   order_manager,
...
int main(int argc, char* argv[]) {
    ...
    if (command == "run") {
        if (argc < 4 || std::string(argv[2]) != "--strategy" || std::string(argv[3]) != "factor") {
            std::cerr << "Invalid command. Use: " << argv[0] << " run --strategy factor [OPTIONS]"
                      << std::endl;
            return 1;
        }
        ...
        if (!parse_command_line(argc, argv, config_file, data_file, symbol, weights_dir,
                                min_threshold, rebalance_time)) {
            return 0; // Help was printed
        }

        return run_factor_strategy(config_file, data_file, symbol, weights_dir, min_threshold,
                                   rebalance_time);
    } else {
