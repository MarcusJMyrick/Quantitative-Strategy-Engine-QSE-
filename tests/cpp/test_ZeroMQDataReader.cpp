...
TEST_F(ZeroMQDataReaderTest, CanCreateZeroMQDataReader) {
    EXPECT_NO_THROW({ ZeroMQDataReader reader("tcp://localhost:5560"); });
}
...
TEST_F(ZeroMQDataReaderTest, ProtocolBuffersSerializationTest) {
    ...
    auto timestamp_s =
        std::chrono::duration_cast<std::chrono::seconds>(original_tick.timestamp.time_since_epoch())
            .count();
    ...
    deserialized_tick.timestamp =
        std::chrono::system_clock::from_time_t(deserialized_proto.timestamp_s());
    ...
    // Note: Timestamp comparison might be tricky due to precision, so we'll just check they're
    // close
    auto time_diff = std::abs(std::chrono::duration_cast<std::chrono::seconds>(
                                  original_tick.timestamp - deserialized_tick.timestamp)
                                  .count());
