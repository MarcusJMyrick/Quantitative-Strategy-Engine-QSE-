...
int main() {
    try {
        ...
        auto order_manager = std::make_unique<qse::OrderManager>(initial_capital, "test_equity.csv",
                                                                 "test_tradelog.csv");
        auto strategy =
            std::make_unique<qse::SMACrossoverStrategy>(order_manager.get(), 10, 20, symbol);
        ...
        std::cout << "Ticks per second: " << (1000.0 / (elapsed_time.count() / 1000.0))
                  << std::endl;
        std::cout << "----------------------------------------" << std::endl;
