...
struct FactorData {
    ...
    FactorData() = default;
    FactorData(const std::string& sym, Timestamp ts, double mom, double vol, double val,
               double comp)
        : symbol(sym), timestamp(ts), momentum(mom), volatility(vol), value(val),
 ...
 * @brief Implements a multi-factor model strategy with factor computation and portfolio
 * construction
 *
...
public:
     ...
     */
    MultiFactorStrategy(const std::vector<std::string>& symbols, int momentum_window = 252,
                        int momentum_short_window = 21, int volatility_window = 20,
                        int value_window = 60, int rebalance_frequency = 5, int top_n = 10,
                        int bottom_n = 10, std::shared_ptr<IOrderManager> order_manager = nullptr);
...
private:
    ...
    // Moving average utilities for factor computation (using pointers to avoid default constructor
    // issues)
