...
TEST_F(SlippageTest, MarketBuyOrderWithSlippage) {
    auto order_manager =
        std::make_unique<qse::OrderManager>(config_, "test_equity.csv", "test_tradelog.csv");
...
TEST_F(SlippageTest, LimitOrderWithSlippage) {
    ...
    auto order_id = order_manager->submit_limit_order("TEST", qse::Order::Side::BUY, 100, 50.0,
