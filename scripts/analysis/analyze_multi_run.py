...
def analyze_single_symbol(equity_file, tradelog_file, symbol):
    ...
    try:
        ...
        if equity_df["returns"].std() > 0:
            sharpe_ratio = (
                equity_df["returns"].mean() / equity_df["returns"].std() * np.sqrt(252 * 24 * 60)
            )  # Assuming minute data
        else:
            sharpe_ratio = 0
...
def create_individual_plots(results, timestamp, output_dir):
    ...
    for symbol_data in results:
        ...
        # Add statistics
        stats_text = (
            f'Initial: ${symbol_data["initial_value"]:,.0f}\n'
...
def create_performance_comparison(results, timestamp, output_dir):
    ...
    axes[0, 1].bar(
        symbols, sharpe_ratios, color=["green" if s >= 0 else "red" for s in sharpe_ratios]
    )
...
def print_summary_table(results, timestamp):
    ...
    print(
        f"{'Symbol':<8} {'Initial':<12} {'Final':<12} {'Return%':<10} {'Sharpe':<8} {'MaxDD%':<8} {'Trades':<8}"
    )
    print("-" * 80)

    for r in valid_results:
        print(
            f"{r['symbol']:<8} ${r['initial_value']:<11,.0f} ${r['final_value']:<11,.0f} "
    ...
    print("-" * 80)
    print(
        f"{'TOTAL':<8} ${total_initial:<11,.0f} ${total_final:<11,.0f} "
