...
TEST_F(BacktesterTest, CanCreateBacktesterAndRun) {
    ...
    qse::Backtester backtester("TEST_SYMBOL", std::unique_ptr<qse::IDataReader>(data_reader_),
...
TEST_F(BacktesterTest, CanRunBacktestWithRealComponents) {
    ...
    auto real_order_manager =
        std::make_unique<qse::OrderManager>(100000.0, "test_equity.csv", "test_tradelog.csv");

    // FIX: Add a bar_duration (e.g., 24 hours) as the fourth argument
    auto real_strategy = std::make_unique<qse::SMACrossoverStrategy>(real_order_manager.get(), 10,
                                                                     20, "TEST_SYMBOL");

    qse::Backtester backtester("TEST_SYMBOL", std::move(data_reader), std::move(real_strategy),
                               std::move(real_order_manager), std::chrono::seconds(60));
...
protected:
    void SetUp() override {
        ...
        test_ticks_ = {{"TEST", qse::from_unix_ms(1000), 100.0, 99.5, 100.5, 1000, 1000, 100},
...
TEST_F(BacktesterTickIntegrationTest, TickStreamIntegration) {
    ...
    // Set expectations
    EXPECT_CALL(*mock_data_reader, read_all_ticks()).WillOnce(ReturnRef(test_ticks_));

    // Expect fill callback to be set
    EXPECT_CALL(*mock_order_manager, set_fill_callback(_)).Times(1);

    // Expect strategy to receive each tick (use flexible matching)
    EXPECT_CALL(*mock_strategy, on_tick(_)).Times(test_ticks_.size());

    // Expect order manager to process each tick (use flexible matching)
    EXPECT_CALL(*mock_order_manager, process_tick(_)).Times(test_ticks_.size());

    // Expect attempt_fills to be called after each tick
    EXPECT_CALL(*mock_order_manager, attempt_fills()).Times(test_ticks_.size());

    // Expect the equity curve to be marked to market after each tick
    EXPECT_CALL(*mock_order_manager, record_equity(_, _)).Times(test_ticks_.size());
    ...
    EXPECT_CALL(*mock_order_manager, get_cash()).WillOnce(Return(10000.0));
    EXPECT_CALL(*mock_order_manager, get_position("TEST")).WillOnce(Return(0));

    // Create and run backtester
    qse::Backtester backtester("TEST", std::move(mock_data_reader), std::move(mock_strategy),
                               std::move(mock_order_manager), std::chrono::seconds(60));
...
TEST_F(BacktesterTickIntegrationTest, EmptyTickStream) {
    ...
    EXPECT_CALL(*mock_data_reader, read_all_ticks()).WillOnce(ReturnRef(empty_ticks_));

    // Expect fill callback to be set
    EXPECT_CALL(*mock_order_manager, set_fill_callback(_)).Times(1);

    // Strategy should not be called with any ticks
    EXPECT_CALL(*mock_strategy, on_tick(_)).Times(0);

    // No attempt_fills calls for empty tick stream
    EXPECT_CALL(*mock_order_manager, attempt_fills()).Times(0);

    // No process_tick calls for empty tick stream
    EXPECT_CALL(*mock_order_manager, process_tick(_)).Times(0);

    // No bars should be generated when there are no ticks
    EXPECT_CALL(*mock_strategy, on_bar(_)).Times(0);
...
TEST_F(BacktesterTickIntegrationTest, RealCSVDataReaderIntegration) {
    ...
    std::string test_csv_content = "timestamp,price,volume\n"
                                   "1000,100.0,100\n"
                                   "1001,100.5,150\n"
                                   "1002,101.0,200\n"
                                   "1003,100.8,120\n"
    ...
    auto order_manager =
        std::make_unique<qse::OrderManager>(10000.0, "test_equity.csv", "test_tradelog.csv");
    auto strategy =
        std::make_unique<qse::SMACrossoverStrategy>(order_manager.get(), 5, 10, "TEST_SYMBOL");

    // Create and run backtester
    qse::Backtester backtester("TEST", std::move(data_reader), std::move(strategy),
...
TEST_F(BacktesterTickIntegrationTest, TickProcessingOrder) {
    ...
    // Expect strategy to receive bars when they're completed
    EXPECT_CALL(*mock_strategy, on_bar(_)).Times(::testing::AtLeast(1));
...
TEST_F(BacktesterTickIntegrationTest, LargeTickStream) {
    ...
    for (int i = 0; i < 1000; ++i) {
        large_tick_stream.push_back({"TEST", qse::from_unix_ms(1000 + i), 100.0 + (i * 0.01),
                                     99.5 + (i * 0.01),  // bid
                                     100.5 + (i * 0.01), // ask
                                     1000,               // bid_size
                                     1000,               // ask_size
    ...
    EXPECT_CALL(*mock_data_reader, read_all_ticks()).WillOnce(ReturnRef(large_tick_stream));

    // Expect fill callback to be set
    EXPECT_CALL(*mock_order_manager, set_fill_callback(_)).Times(1);

    // Expect all ticks to be processed (use flexible matching)
    EXPECT_CALL(*mock_strategy, on_tick(_)).Times(1000);
    EXPECT_CALL(*mock_order_manager, process_tick(_)).Times(1000);
    EXPECT_CALL(*mock_order_manager, attempt_fills()).Times(1000);
    EXPECT_CALL(*mock_order_manager, record_equity(_, _)).Times(1000);
...
TEST_F(BacktesterTickIntegrationTest, DebugMockWiring) {
    ...
    EXPECT_CALL(*mock_strategy, on_tick(_)).Times(::testing::AnyNumber());

    // Expect order manager to process ticks (any number)
    EXPECT_CALL(*mock_order_manager, process_tick(_)).Times(::testing::AnyNumber());

    // Expect attempt_fills to be called (any number)
    EXPECT_CALL(*mock_order_manager, attempt_fills()).Times(::testing::AnyNumber());

    // Expect strategy to receive bars (any number)
    EXPECT_CALL(*mock_strategy, on_bar(_)).Times(::testing::AnyNumber());
