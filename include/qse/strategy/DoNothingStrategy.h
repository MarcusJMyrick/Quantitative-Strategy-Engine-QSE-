...
public:
    ...
    void on_tick(const qse::Tick& tick) override {
        ...
        if (tick_count_ % 100 == 0) { // Log every 100th tick to avoid spam
            std::cout << "[DoNothingStrategy] Received tick #" << tick_count_ << " for "
                      << tick.symbol << " at $" << tick.price << std::endl;
        }
    }
    ...
    void on_bar(const qse::Bar& bar) override {
        bar_count_++;
        std::cout << "[DoNothingStrategy] Received bar #" << bar_count_ << " OHLC: $" << bar.open
                  << "/" << bar.high << "/" << bar.low << "/" << bar.close << " Vol: " << bar.volume
                  << std::endl;
    }
    ...
    void on_fill(const qse::Fill& fill) override {
        fill_count_++;
        std::cout << "[DoNothingStrategy] Received fill #" << fill_count_ << " for " << fill.symbol
                  << " " << fill.side << " " << fill.quantity << " @ $" << fill.price << std::endl;
    }
