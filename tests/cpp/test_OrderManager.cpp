...
TEST_F(OrderManagerTest, EquityRecordingWithMultipleSymbols) {
    {
        ...
        std::map<std::string, double> market_prices = {
            {"AAPL", 160.0}, {"GOOG", 2600.0}, {"MSFT", 280.0}};
        manager.record_equity(1234567890, market_prices);
...
TEST_F(OrderManagerTest, FileOutputFormat) {
    {
        ...
        // Record equity
        std::map<std::string, double> market_prices = {{"AAPL", 160.0}, {"GOOG", 2600.0}};
        manager.record_equity(1234567890, market_prices);
    }
...
protected:
    void SetUp() override {
        ...
        test_ticks_ = {
            ...
            {"AAPL", qse::from_unix_ms(1002), 100.1, 99.9, 100.0, 200, 200,
             200}, // mid: 100.1, ask=100.0 allows buy limit at 100.0 to fill
        };
    }
...
TEST_F(OrderManagerTickSimulationTest, LimitBuyOrderFillsOnCross) {
    ...
    auto order_id = order_manager->submit_limit_order("AAPL", qse::Order::Side::BUY, 100, 100.0,
...
TEST_F(OrderManagerTickSimulationTest, LimitSellOrderFillsOnCross) {
    ...
    auto order_id = order_manager->submit_limit_order("AAPL", qse::Order::Side::SELL, 50, 100.2,
                                                      qse::Order::TimeInForce::GTC);
    ...
    qse::Tick cross_tick{"AAPL", qse::from_unix_ms(1003), 100.3, 100.2, 100.4, 100, 100,
                         100}; // bid=100.2 allows sell limit at 100.2 to fill
...
TEST_F(OrderManagerTickSimulationTest, IOCOrderCancelsIfNotFilled) {
    ...
    auto order_id = order_manager->submit_limit_order("AAPL", qse::Order::Side::BUY, 100, 99.0,
                                                      qse::Order::TimeInForce::IOC);
...
TEST_F(OrderManagerTickSimulationTest, IOCOrderFillsIfPriceCrosses) {
    ...
    qse::Tick fill_tick{"AAPL", qse::from_unix_ms(1000), 100.0, 99.5, 100.0, 100, 100,
                        100}; // ask=100.0 allows buy limit at 100.0 to fill
...
TEST_F(OrderManagerTickSimulationTest, GetActiveOrders) {
    ...
    auto order1 = order_manager->submit_limit_order("AAPL", qse::Order::Side::BUY, 100, 99.0,
                                                    qse::Order::TimeInForce::GTC);
    auto order2 = order_manager->submit_limit_order("AAPL", qse::Order::Side::SELL, 50, 101.0,
                                                    qse::Order::TimeInForce::GTC);
    auto order3 = order_manager->submit_limit_order("GOOGL", qse::Order::Side::BUY, 200, 150.0,
                                                    qse::Order::TimeInForce::GTC);
...
TEST_F(OrderManagerTickSimulationTest, PartialFills) {
    ...
    auto order_id = order_manager->submit_limit_order("AAPL", qse::Order::Side::BUY, 1000, 100.0,
                                                      qse::Order::TimeInForce::GTC);

    // Create a tick with limited volume and ask that crosses the limit
    qse::Tick limited_tick{"AAPL", qse::from_unix_ms(1000), 100.0, 99.5, 100.0, 500, 500,
                           500}; // ask=100.0 allows buy limit at 100.0 to fill, but only 500 volume
...
TEST_F(OrderManagerTickSimulationTest, MultipleOrdersOnSameTick) {
    ...
    // Submit multiple orders
    auto buy_order = order_manager->submit_limit_order("AAPL", qse::Order::Side::BUY, 100, 100.0,
                                                       qse::Order::TimeInForce::GTC);
    auto sell_order = order_manager->submit_limit_order("AAPL", qse::Order::Side::SELL, 50, 100.0,
                                                        qse::Order::TimeInForce::GTC);
    ...
    qse::Tick cross_tick{"AAPL", qse::from_unix_ms(1000), 100.0, 100.0, 100.0, 200, 200,
                         200}; // bid=ask=100.0 allows both buy and sell limits at 100.0 to fill
    order_manager->process_tick(cross_tick);
